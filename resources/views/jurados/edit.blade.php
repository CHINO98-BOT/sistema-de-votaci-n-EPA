...
@section('title', 'Editar Asignación de Jurado')
...
<div class="card">
    ...
    <form action="{{ route('jurados.update', $jurado->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="form-group">
                ...
                <select name="event_id" id="event_id" class="form-control @error('event_id') is-invalid @enderror" required>
                    ...
                    @foreach($eventos as $evento)
                        <option value="{{ $evento->id }}" {{ $jurado->event_id == $evento->id ? 'selected' : '' }}>
                            {{ $evento->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="juror_user_id" id="juror_user_id" class="form-control @error('juror_user_id') is-invalid @enderror" required>
                    ...
                    @foreach($usuarios as $usuario)
                        <option value="{{ $usuario->id }}" {{ $jurado->juror_user_id == $usuario->id ? 'selected' : '' }}>
